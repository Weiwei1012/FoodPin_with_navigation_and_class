...
class RestaurantTableViewController: UITableViewController {
    ...
    var restaurants:[Restaurant] = []           //empty array
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        Restaurant.generateData(sourceArray: &restaurants)  // init array (restaurants) class name call class mexer
                                                            // & memory address
...
//        tableView.deselectRow(at: indexPath, animated: false)
    ...
    // MARK: - Navigation
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {         //轉場：傳遞 data by using segue
        if segue.identifier == "showRestaurantDetail" {
            if let indexPath = tableView.indexPathForSelectedRow {
                let destinationController = segue.destination as! RestaurantDetailViewController
                destinationController.restaurant = restaurants[indexPath.row]
                //destinationController.restaurantImageView.image = UIImage(named: restaurantImages[indexPath.row])     // can't pass to IBOutlet variable
            }
        }
    }
